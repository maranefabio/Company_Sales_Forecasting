...
def setup_logger(output_path: Path | None = None) -> None:
    ...
    # Args:
    #   - output_path: When provided, a "RotatingFileHandler" writing to
    #     "{output_path}/logs/log_{YYYYMMDD_HHMMSS}.txt" (max 1 MiB per file, 3 backups kept) is
    #     added at DEBUG level. When None, only the console handler (INFO level) is configured.

    # Returns:
    #   None. This function only has the side effect of mutating the root logger's handlers.
