...
class Database:
    ...
    def __init__(self, database_settings: DatabaseSettings) -> None:
        # Initialize database wrapper instance.

        # Args:
        #   - database_settings: Settings object bundling the database connection info (ODBC
        #     connection string, plus server and database names used only for logging).
    ...
    def connect(self, max_retries: int = 3) -> Generator[pyodbc.Connection, None]:
        ...
        #   - If a connection attempt fails before "connection" is (re)assigned to a valid object
        #     (e.g. "pyodbc.connect" itself raises on the first attempt), the "finally" clause will
        #     still attempt to close "connection", which may be "None" or an already-closed
        #     connection from a prior failed attempt.
        ...
        # Yields:
        #   - pyodbc.Connection: an open connection that has passed the "SELECT 1" check.
